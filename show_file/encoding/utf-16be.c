...
{ 
	...
	if (strncmp((const char*)pucBufHead, (const char*)aStrUtf16be, 2) == 0){
		return 1;
	}	
	return 0;		
}
...
int Utf16beEncodingInit(void)
{
	...
	AddFontOprForEncoding(&g_tUtf16beEncodingOpr, GetFontOprForName("hzk"));
	AddFontOprForEncoding(&g_tUtf16beEncodingOpr, GetFontOprForName("freetype"));
