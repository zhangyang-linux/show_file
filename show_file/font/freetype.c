...
static int FreeTypeFontInit(const char* pFileName, int dwFontSize);
static int FreeTypeGetFontBitMap(unsigned int dwCode, PT_FontBitMap ptFontBitMap);
static int FreeTypeExit(struct FontOpr* ptFontOpr);

static T_FontOpr g_tFreeTypeFontOpr = {
	.name = "freetype",
	.FontInit = FreeTypeFontInit,
	.GetFontBitMap = FreeTypeGetFontBitMap,	
	.FontExit = FreeTypeExit,
};
...
static int FreeTypeGetFontBitMap(unsigned int dwCode, PT_FontBitMap ptFontBitMap)
{
	...
	if (iError){
		DBG_PRINTF("FT_Load_Char for code: 0x%x error!\n", dwCode);
		return -1;
	}
	...
	ptFontBitMap->iBpp = 1;	
	ptFontBitMap->iPitch = g_tglyph->bitmap.pitch;
...
static int FreeTypeExit(PT_FontOpr ptFontOpr)
{
	//s(void*)ptFontOpr;
